# Adapted from CT-CLIP's scripts/data_inference_nii.py
# Change: removed the RescaleSlope/RescaleIntercept step since our volumes
# are from the v2 (_fixed) split, which is already HU-corrected.
...
class CTReportDatasetFixed(Dataset):
    ...
    def nii_img_to_tensor(self, path, df):
        ...
        # NOTE: no slope/intercept correction here — v2 (_fixed) volumes
        # are already in HU. Confirmed via sanity check: raw range was
        # -1024 to 3071, exactly right for chest CT HU, no header scaling present.
